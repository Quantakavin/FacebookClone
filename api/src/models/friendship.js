const connection = require('../config/database')

module.exports.insert = async (userid, friendid, status) => {
    const sql =
        'INSERT INTO friendship (user_id, friend_id, status) VALUES ($1, $2, $3)'
    return connection.query(sql, [userid, friendid, status])
}

module.exports.delete = async (userid, friendid) => {
    const sql =
        'DELETE FROM friendship WHERE (user_id = $1 AND friend_id=$2) OR (user_id = $2 AND friend_id=$1)'
    return connection.query(sql, [userid, friendid])
}

module.exports.getFriendship = async (userid, friendid) => {
    const sql =
        'SELECT * FROM friendship WHERE (user_id = $1 AND friend_id=$2) OR (user_id = $2 AND friend_id=$1)'
    return connection.query(sql, [userid, friendid])
}

module.exports.getRequests = async (friend_id) => {
    const sql = `SELECT users.*, users.picurl, users.name, users.id FROM users INNER JOIN friendship ON users.id = friendship.user_id WHERE (friend_id = $1 AND status = 'requested')`
    return connection.query(sql, [friend_id])
}

module.exports.acceptRequests = async (
    status,
    confirmed,
    friend_id,
    user_id
) => {
    const sql = `UPDATE friendship SET status = $1, confirmed = $2 WHERE (friend_id = $3 AND user_id = $4 AND status = 'requested')`
    return connection.query(sql, [status, confirmed, friend_id, user_id])
}

module.exports.declineRequests = async (
    status,
    confirmed,
    friend_id,
    user_id
) => {
    const sql = `UPDATE friendship SET status = $1, confirmed = $2 WHERE (friend_id = $3 AND user_id = $4 AND status = 'requested')`
    return connection.query(sql, [status, confirmed, friend_id, user_id])
}

module.exports.getMutualFriends = async (user_id, friend_id) => {
    const sql = `SELECT * from users WHERE id in (

            (SELECT friend_id from friendship where user_id = $1 UNION SELECT user_id  from friendship where friend_id = $1) 
                    INTERSECT 
            (SELECT friend_id  from friendship where user_id = $2 UNION SELECT user_id  from friendship where friend_id  = $2)

        )`
    return connection.query(sql, [user_id, friend_id])
}

module.exports.getFriendList = async (user_id) => {
    const sql = `SELECT COUNT(*) OVER () AS friendcount, u.id, u.name, u.picurl FROM friendship f INNER JOIN users u ON (f.user_id = u.id OR f.friend_id = u.id)
        where f.status='accepted' And (f.user_id = $1 OR f.friend_id = $1) AND u.id != $1`
    return connection.query(sql, [user_id])
}
