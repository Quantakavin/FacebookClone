...
module.exports.getNotifications = async (receiver_id) => {
    const sql = `SELECT notification.*,notification.data ->> 'userid' AS userid, users.name, notificationmessage.notification_message FROM notification INNER JOIN users ON users.id = CAST((notification.data ->> 'postid') AS INTEGER) INNER JOIN notificationmessage ON notification.notification_id = notificationmessage.id WHERE (notification.receiver_id = $1) order by time desc`
    return new Promise((resolve, reject) => {
        connection
            .query(sql, [receiver_id])
            .then((result) => {
